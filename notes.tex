...
\begin{multline}\label{opest}
 \rho(A\varphi_, A\varphi_2) = \max\limits_{a \leqslant t \leqslant b} |A\varphi_1(t) - A\varphi_2(t)|=\\
...
Нехай оператор А діє з $X$ в $X$. Виберемо будь-який елемент $x \in X$. Тоді $Ax$ буде знову належати простору $X$ і до нього знову можна застосувати оператор $A$: $A(Ax)$. Оператор, що відповідає послідовному застосуванню оператора $A$, будемо позначати символом $A^2$ і називатимемо квадратом оператора $A$\index{квадрат оператора}.
...
\|A\| = \sup_{\|x\| \leqslant 1} \geqslant \|Ax_0\|=b.
\end{equation}
...
x=A^{-1}f
\end{equation}
є розв'язок рівняння \eqref{opeq1}. Неважко показати, що цей розв'язкою є єдиним.
...
\begin{enumerate}
	\item Нехай А -- лінійний оператор, що діє у $\mathbb{R}^n$, і нехай $\mathfrak{U} = (a_ij)_{i,j = \overline{1, n}}$ -- матриця цього оператора.
...
\begin{dfn}
	Множина $D$ функцій $x(t) \in \mathbb{c}([a; b])$ називається рівномірно обмеженою, якщо існує стала $M>0$ така, що $|x(t)| \leqslant M$ для усіх $x(t) \in D$ за будь-якого $t \in [a; b]$.\index{множина!рівномірно обмежена}
\end{dfn}
\begin{dfn}
...
Повернемося до побудови спектра симетричного компактного оператора. Нехай $L_1$ -- підпростір\index{породжений підпростір}, породжений елементом $x_1$, тобто сукупність елементів вигляду $\{tx_1\}$, де $t$ -- числовий параметр. Тоді
\[
...
\begin{dfn}
	Регуляризацією\index{регуляризація} рівняння \eqref{illopeq} називається заміна початкової некоректної задачі сімейством \emph{близьких до неї} коректних задач і наступне наближення до $\hat{x}$ із прямуванням похибки у даних $y$ до нуля.
\end{dfn}
...
I=\underbrace{{h/2} + \dfrac{I_{h/2}-I_h}{2^{p}-1}}_{I_{h,h/2}} + c_2 h^{p+1} + O\left(h^{p+2}\right).
\end{equation}
